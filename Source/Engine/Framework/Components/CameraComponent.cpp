...
namespace nc
{
	...
	void CameraComponent::Update(float dt)
	{
		...
		{
			projection = glm::ortho(size * aspect* .5f, -size * aspect*.5f,-size *.5f, size * .5f, near, far);
		}
	}
	...
	void CameraComponent::ProcessGui()
	{
		...
		// Use ImGui::DragFloat to set fov, aspect, near, and far values with a speed of 0.1f
		ImGui::DragFloat("Field of View (FOV)", &fov, 0.1f, 0.1f, 179.9f); // Adjust the range and step as needed
	...
	void CameraComponent::Read(const json_t& value)
	{
		...
		if (StringUtils::IsEqualIgnoreCase("orthographic", projectionTypeName)) projectionTypeName = Orthographic;
